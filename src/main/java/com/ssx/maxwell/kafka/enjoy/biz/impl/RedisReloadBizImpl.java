...
public class RedisReloadBizImpl implements RedisReloadBiz {
                ...
                if (!Strings.isNullOrEmpty(rule) && !MaxwellBinlogConstants.REDIS_RULE_0.equals(rule)) {
                    ...
                    if (ArrayUtils.isNotEmpty(ruleArr)) {
                        if (ArrayUtils.contains(ruleArr, MaxwellBinlogConstants.REDIS_RULE_1)) {
                            ...
                            executeToRedis(loopGetDynamicDsInfo(dbDatabase), jdbcSql, redisKey, redisMapping, false);
                        }
                        ...
                        if (ArrayUtils.contains(ruleArr, MaxwellBinlogConstants.REDIS_RULE_3)) {
                            ...
                            log.info("sql= {} , key= {}", jdbcSql, redisKey);
                            executeToRedis(loopGetDynamicDsInfo(dbDatabase), jdbcSql, redisKey, redisMapping, true);
     ...
     * 功能描述: 执行至redis业务
     *
     * @param: [dynamicDsInfo, sql, redisKey, redisMapping, isCustom]
     * @return: void
     * @author: shuaishuai.xiao
     * @date: 2019/6/14 17:31
     */
    private void executeToRedis(DynamicDsInfo dynamicDsInfo, String sql, String redisKey, RedisMapping redisMapping, boolean isCustom) {
        ...
        if (isCustom) {
                        ...
                        for (String part : templateArr) {
                            ...
                            for (String f : a2) {
                                if (!Strings.isNullOrEmpty(f)) {
                                    keyBuilder.append(":");
                                    Object dbObj = stringObjectMap.get(f);
                            ...
                            log.info("自定义缓存,key={}", finalKey);
                            setValueToRedis(redisMapping, finalKey, buildCacheListDTO(dbDataList));
                        }
                    }
                }
            ...
            if (null == dbDataList || dbDataList.isEmpty()) {
                ...
                setValueToRedis(redisMapping, redisKey, buildCacheListDTO(dbDataList));
            }
        }
     ...
     * 功能描述: set to redis
     *
     * @param: [redisMapping, redisKey, cacheListDTO]
     * @return: void
     * @author: shuaishuai.xiao
